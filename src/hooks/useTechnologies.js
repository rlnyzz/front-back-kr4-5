...
function useTechnologies() {
  ...
  // Если хотите использовать уведомления внутри хука:
  ...
  const updateStatus = useCallback((techId, newStatus) => {
    ...
    // showSuccess(`Статус технологии обновлён: ${newStatus}`);
  }, [setTechnologies]);
  ...
  // ФУНКЦИЯ editTechnology - БЫЛА ПРОПУЩЕНА СКОБКА!
  ...
  const deleteTechnology = useCallback((techId) => {
    ...
    // showWarning('Технология удалена');
  }, [setTechnologies]);
  ...
  // Функция для сохранения настроек фильтрации в localStorage
  const [filterSettings, setFilterSettings] = useLocalStorage('tech-filter-settings', {
    status: '',
    category: '',
    difficulty: '',
    sortBy: 'createdAt',
  ...
  const updateFilterSettings = useCallback((newSettings) => {
    setFilterSettings(prev => ({ ...prev, ...newSettings }));
  }, [setFilterSettings]);

  // Расширенная фильтрация с учётом сохранённых настроек
  const filterTechnologiesExtended = useCallback((customFilters = {}) => {
    const filters = { ...filterSettings, ...customFilters };
  ...
  return {
    ...
    updateNotes,
    addTechnology,
    editTechnology, // Теперь функция определена корректно
    deleteTechnology,
    markAllCompleted,
    resetAllStatuses,
    importTechnologies,
